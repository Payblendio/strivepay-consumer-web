...
export function TransactionRealtimeRefresh({scope}:{scope:string}){
  ...
  useEffect(()=>connectSharedSupport({scope,requestTicket:async signal=>{const response=await customerFetch("/api/support/socket-ticket",{method:"POST",signal,headers:{"X-StrivePay-Account-Scope":scope}});if(response.status===401||response.status===403||response.status===409||response.status===400)return null;if(!response.ok)throw new Error("Realtime connection unavailable");return await response.json() as SupportConnectionTicket;},onChange:()=>{},onTransactionChange:()=>router.refresh(),onState:()=>{}}),[router,scope]);
  return null;
}
