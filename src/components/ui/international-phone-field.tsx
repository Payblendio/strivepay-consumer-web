...
export function InternationalPhoneField({country,value,onCountryChange,onChange,countryError,error,allowedCountries,className=""}:{country?:Country;value?:Value;onCountryChange:(country:Country)=>void;onChange:(value:Value|undefined)=>void;countryError?:string;error?:string;allowedCountries?:Country[];className?:string}){
  ...
  const residenceOptions=useMemo(()=>{const allowed=allowedCountries?new Set(allowedCountries):null;return filterCountries(residenceQuery).filter(item=>!allowed||allowed.has(item.code))},[residenceQuery,allowedCountries]),phoneOptions=useMemo(()=>filterCountries(phoneQuery),[phoneQuery]);
  ...
  return <div className={`international-phone-fields ${className}`.trim()} onPasteCapture={pasteInternationalPhone}>
    ...
    <CountryModal open={residenceOpen} onClose={()=>{setResidenceOpen(false);setResidenceQuery("");}} title="Country of residence" description="Choose the country where you currently live." query={residenceQuery} setQuery={setResidenceQuery} options={residenceOptions} emptyMessage={allowedCountries&&allowedCountries.length===0?"Loading the countries we support…":undefined} selected={country} onSelect={next=>{onCountryChange(next);setResidenceOpen(false);setResidenceQuery("")}} className={modalClassName}/>
...
function CountryModal({open,onClose,title,description,query,setQuery,options,selected,onSelect,emptyMessage,className=""}:{open:boolean;onClose:()=>void;title:string;description:string;query:string;setQuery:(value:string)=>void;options:typeof countries;selected?:Country;onSelect:(country:Country)=>void;emptyMessage?:string;className?:string}){
  ...
  return <Modal open={open} onClose={onClose} title={title} description={description} size="large" className={className}>
    ...
    {options.length===0?<p className="route-option-empty" role="status">{emptyMessage??"No countries match your search. Try a name, country code or dial prefix."}</p>:<div className="country-grid phone-country-grid">{options.map(item=><button type="button" key={item.code} className={item.code===selected?"selected":""} aria-pressed={item.code===selected} onClick={()=>onSelect(item.code)}><span className="country-flag small"><CircleFlag countryCode={item.code.toLowerCase()} height="36"/></span><span><strong>{item.name}</strong>{" "}<small>{item.code} · {item.dial}</small></span>{item.code===selected&&<IconCheck className="country-check" size={19}/>}</button>)}</div>}
  </Modal>;
}
